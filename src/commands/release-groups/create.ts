...
export default class ReleaseGroupsCreate extends BaseCommand {
  public static override description = 'Create a Productboard release group'
  ...
  public override async run(): Promise<void> {
    const {  flags } = await this.parse(ReleaseGroupsCreate)
    const runtime = await this.initRuntime(flags)

    const body = parseJsonObject(flags.data, '--data')
    const endpoint = '/release-groups'
