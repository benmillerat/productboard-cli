...
export default class NotesTagsList extends BaseCommand {
  public static override description = 'List Productboard tag links'
  ...
  public override async run(): Promise<void> {
    const { args, flags } = await this.parse(NotesTagsList)
    const runtime = await this.initRuntime(flags)

    const limit = flags.all ? 1000 : flags.limit
    const query = parseKeyValuePairs(flags.query, '--query')
    const endpoint = `/notes/${encodeURIComponent(args.noteId)}/tags`
