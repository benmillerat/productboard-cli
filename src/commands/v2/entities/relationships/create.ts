...
export default class V2EntitiesRelationshipsCreate extends V2BaseCommand {
  public static override description = 'Create a Productboard v2 beta entity relationship'
  ...
  public override async run(): Promise<void> {
    const { args, flags } = await this.parse(V2EntitiesRelationshipsCreate)
    const runtime = await this.initRuntime(flags)

    const body = parseJsonObject(flags.data, '--data')
    const endpoint = `/v2/entities/${encodeURIComponent(args.id)}/relationships`
