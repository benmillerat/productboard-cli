...
export default class V2EntitiesConfigurationsGet extends V2BaseCommand {
  public static override description = 'Get a Productboard v2 beta entity configuration'
  ...
  public override async run(): Promise<void> {
    const { args, flags } = await this.parse(V2EntitiesConfigurationsGet)
    const runtime = await this.initRuntime(flags)

    const endpoint = `/v2/entities/configurations/${encodeURIComponent(args.type)}`
