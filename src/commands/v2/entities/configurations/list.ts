...
export default class V2EntitiesConfigurationsList extends V2BaseCommand {
  public static override description = 'List Productboard v2 beta entity configurations'
  ...
  public override async run(): Promise<void> {
    const { flags } = await this.parse(V2EntitiesConfigurationsList)
    const runtime = await this.initRuntime(flags)

    const limit = flags.all ? 1000 : flags.limit
    const query = parseKeyValuePairs(flags.query, '--query')
    const endpoint = '/v2/entities/configurations'
