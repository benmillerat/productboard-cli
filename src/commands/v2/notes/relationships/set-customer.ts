...
export default class V2NotesRelationshipsSetCustomer extends V2BaseCommand {
  public static override description = 'Set Productboard v2 beta note customer relationship'
  ...
  public override async run(): Promise<void> {
    const { args, flags } = await this.parse(V2NotesRelationshipsSetCustomer)
    const runtime = await this.initRuntime(flags)

    const body = parseJsonObject(flags.data, '--data')
    const endpoint = `/v2/notes/${encodeURIComponent(args.id)}/relationships/customer`
