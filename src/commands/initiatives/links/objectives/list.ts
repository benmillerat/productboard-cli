...
export default class InitiativesLinksObjectivesList extends BaseCommand {
  public static override description = 'List Productboard objective links'
  ...
  public override async run(): Promise<void> {
    const { args, flags } = await this.parse(InitiativesLinksObjectivesList)
    const runtime = await this.initRuntime(flags)

    const limit = flags.all ? 1000 : flags.limit
    const query = parseKeyValuePairs(flags.query, '--query')
    const endpoint = `/initiatives/${encodeURIComponent(args.initiativeId)}/links/objectives`
    ...
    outputResult(this.log, payload, {
      ...
      quiet: runtime.quiet,
      quietValue: quietIds,
      resource: 'initiatives/links/objectives',
      wide: runtime.wide,
