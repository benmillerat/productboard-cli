...
export default class InitiativesLinksFeaturesList extends BaseCommand {
  public static override description = 'List Productboard feature links'
  ...
  public override async run(): Promise<void> {
    const { args, flags } = await this.parse(InitiativesLinksFeaturesList)
    const runtime = await this.initRuntime(flags)

    const limit = flags.all ? 1000 : flags.limit
    const query = parseKeyValuePairs(flags.query, '--query')
    const endpoint = `/initiatives/${encodeURIComponent(args.initiativeId)}/links/features`
    ...
    outputResult(this.log, payload, {
      ...
      quiet: runtime.quiet,
      quietValue: quietIds,
      resource: 'initiatives/links/features',
      wide: runtime.wide,
