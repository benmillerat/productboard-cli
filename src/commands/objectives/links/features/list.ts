...
export default class ObjectivesLinksFeaturesList extends BaseCommand {
  ...
  public override async run(): Promise<void> {
    const { args, flags } = await this.parse(ObjectivesLinksFeaturesList)
    const runtime = await this.initRuntime(flags)

    const limit = flags.all ? 1000 : flags.limit
    const query = parseKeyValuePairs(flags.query, '--query')
    const endpoint = `/objectives/${encodeURIComponent(args.objectiveId)}/links/features`
    ...
    outputResult(this.log, payload, {
      ...
      quiet: runtime.quiet,
      quietValue: quietIds,
      resource: 'objectives/links/features',
      wide: runtime.wide,
