...
export default class ObjectivesLinksInitiativesList extends BaseCommand {
  public static override description = 'List Productboard initiative links'
  ...
  public override async run(): Promise<void> {
    const { args, flags } = await this.parse(ObjectivesLinksInitiativesList)
    const runtime = await this.initRuntime(flags)

    const limit = flags.all ? 1000 : flags.limit
    const query = parseKeyValuePairs(flags.query, '--query')
    const endpoint = `/objectives/${encodeURIComponent(args.objectiveId)}/links/initiatives`
    ...
    outputResult(this.log, payload, {
      ...
      quiet: runtime.quiet,
      quietValue: quietIds,
      resource: 'objectives/links/initiatives',
      wide: runtime.wide,
