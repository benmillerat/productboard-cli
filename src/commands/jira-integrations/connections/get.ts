...
export default class JiraIntegrationsConnectionsGet extends BaseCommand {
  public static override description = 'Get a Productboard Jira integration connection'
  ...
  public override async run(): Promise<void> {
    const { args, flags } = await this.parse(JiraIntegrationsConnectionsGet)
    const runtime = await this.initRuntime(flags)

    const endpoint = `/jira-integrations/${encodeURIComponent(args.id)}/connections/${encodeURIComponent(args.featureId)}`
