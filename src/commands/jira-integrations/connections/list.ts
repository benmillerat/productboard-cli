...
export default class JiraIntegrationsConnectionsList extends BaseCommand {
  public static override description = 'List Productboard Jira integration connections'
  ...
  public override async run(): Promise<void> {
    const { args, flags } = await this.parse(JiraIntegrationsConnectionsList)
    const runtime = await this.initRuntime(flags)

    const limit = flags.all ? 1000 : flags.limit
    const query = parseKeyValuePairs(flags.query, '--query')
    const endpoint = `/jira-integrations/${encodeURIComponent(args.id)}/connections`
