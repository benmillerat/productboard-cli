...
export default class FeedbackConfigurationsList extends BaseCommand {
  public static override description = 'List Productboard feedback configurations'
  ...
  public override async run(): Promise<void> {
    const {  flags } = await this.parse(FeedbackConfigurationsList)
    const runtime = await this.initRuntime(flags)

    const limit = flags.all ? 1000 : flags.limit
    const query = parseKeyValuePairs(flags.query, '--query')
    const endpoint = '/feedback-form-configurations'
    ...
    outputResult(this.log, payload, {
      ...
      quiet: runtime.quiet,
      quietValue: quietIds,
      resource: 'feedback-configurations',
      wide: runtime.wide,
