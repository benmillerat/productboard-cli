...
export default class ComponentsUpdate extends BaseCommand {
  public static override description = 'Update a Productboard component'
  public static override args = {
    id: Args.string({ description: 'Component ID', required: true }),
  }
  ...
  public override async run(): Promise<void> {
    const { args, flags } = await this.parse(ComponentsUpdate)
    const runtime = await this.initRuntime(flags)

    const body = parseJsonObject(flags.data, '--data')
    const endpoint = `/components/${encodeURIComponent(args.id)}`
