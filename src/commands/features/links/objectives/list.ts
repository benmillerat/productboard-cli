...
export default class FeaturesLinksObjectivesList extends BaseCommand {
  ...
  public override async run(): Promise<void> {
    const { args, flags } = await this.parse(FeaturesLinksObjectivesList)
    const runtime = await this.initRuntime(flags)

    const limit = flags.all ? 1000 : flags.limit
    const query = parseKeyValuePairs(flags.query, '--query')
    const endpoint = `/features/${encodeURIComponent(args.featureId)}/links/objectives`
    ...
    outputResult(this.log, payload, {
      ...
      quiet: runtime.quiet,
      quietValue: quietIds,
      resource: 'features/links/objectives',
      wide: runtime.wide,
